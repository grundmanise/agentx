...
const (
	...
	installChildArgs = "AGENTX_TEST_INSTALL_ARGS"
)
...
func TestInstallChildProcess(t *testing.T) {
	...
	args := append([]string{"skill", "add", os.Getenv(installChildURL)}, strings.Fields(os.Getenv(installChildArgs))...)
	code := Run(context.Background(), args, env, strings.NewReader(""), os.Stdout, os.Stderr)
	os.Exit(code)
}

// killedInstall runs one install in a child process that a git wrapper
// kills the moment the import branches are written, which is a durable
// boundary: the refs are live, the journal describes what is left, and
// nothing has recorded that the refs went in. It returns what the child
// printed, for a test that has to say where it was killed.
func killedInstall(t *testing.T, h *harness, s *sourceRepo, args ...string) string {
	t.Helper()
	...
	// before it could record that it made it. A deletion is not held, so the
	// cleanup of an earlier run's staging refs never triggers it.
	stubGit(t, h, `#!/bin/sh
case " $* " in
*" update-ref "*)
	`+real+` "$@"
...
`)
	...
	child.Env = append(os.Environ(), installChildEnv+"=1", installChildURL+"="+s.url, installChildArgs+"="+strings.Join(args, " "))
	for k, v := range h.env {
...
func TestSkillAddRecoversFromAKilledRun(t *testing.T) {
	...
	out := killedInstall(t, h, s, "--skill", "alpha")
...
// TestSkillAddRecoversFromAKilledBatch is the same boundary for a batch of
// several skills, which is one journal and not one per skill: the run is
// killed with every import branch written and no library directory made,
// and the next command finishes all of them. A batch that left one journal
// per skill would recover each of them on its own and could stop between
// two, leaving the machine holding branches for skills it does not have.
func TestSkillAddRecoversFromAKilledBatch(t *testing.T) {
	t.Parallel()
	h, s := bulkHarness(t)
	out := killedInstall(t, h, s, "--all")
	...
	// One journal for the whole batch, whatever it holds.
	if n := journalCount(t, h); n != 1 {
		t.Fatalf("%d journals after the killed batch, want 1:\n%s", n, out)
	}
