...
func TestColorResolve(t *testing.T) {
	...
	tests := []struct {
		...
		{"auto on a pipe", colorAuto, map[string]string{}, false},
		{"always on a pipe", colorAlways, map[string]string{}, true},
		{"always despite NO_COLOR", colorAlways, map[string]string{"NO_COLOR": "1"}, true},
		{"never", colorNever, map[string]string{}, false},
		{"NO_COLOR set empty", colorAuto, map[string]string{"NO_COLOR": ""}, false},
		{"TERM dumb", colorAuto, map[string]string{"TERM": "dumb"}, false},
	}
...
func TestColorKeepsAlignment(t *testing.T) {
	...
	for _, args := range [][]string{{"config", "list"}, {"doctor"}, {"help"}, {"machine"}} {
		plain := h.run(args...)
		painted := h.run(append([]string{"--color=always"}, args...)...)
...
func TestColorRejectsUnknownMode(t *testing.T) {
	t.Parallel()
	h := newHarness(t)
	out := h.run("--color", "loud", "version")
	equal(t, "exit", out.exit, 1)
	equal(t, "stdout", out.stdout, "")
	contains(t, "stderr", out.stderr, `error: invalid value "loud" for --color`)
	contains(t, "stderr", out.stderr, "hint: use auto, always or never")

	out = h.run("--json", "--color", "loud", "version")
	equal(t, "exit", out.exit, 1)
...
func TestHelpSections(t *testing.T) {
	...
	for _, s := range []string{
		...
		"Global flags:\n",
		"      --color string  colour the text output: auto, always or never (default auto)\n",
	...
	out = h.run("--color=always", "scan", "--help")
