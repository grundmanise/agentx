...
type HandshakeOptions struct {
	...
	Parallel int               // servers handshaken at once
	Version  string            // the CLI version, told to servers
}

// Handshake connects to every declared server, outside any lock, and
// records what each exposed. A server that cannot be reached, answers
// badly or runs out of time is one warning naming its file and name, never
// its environment or headers, and keeps its stored signature, if any.
func (s *Scan) Handshake(ctx context.Context, o HandshakeOptions) {
	var mu sync.Mutex
	var wg sync.WaitGroup
	slots := make(chan struct{}, max(o.Parallel, 1))
