...
Two invocation modes. One-shot commands perform actions: `scan`, `skill add|fork|update|revert|remove|list`, `source add|list`, `login`, `logout`, `whoami`, `machine`. A long-lived `agentx serve --json` child, spawned once at app startup, runs the sync loop, executes operations queued for this machine, watches the account repo and the library for edits, and streams events until the app exits. The serve child and one-shot commands share the same SQLite database in WAL mode, so a terminal user and the app can act at the same time.
