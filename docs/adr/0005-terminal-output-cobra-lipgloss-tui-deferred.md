...
| Tables | The small `table` type in `internal/cli/ui.go` | `lipgloss/table` pads the last cell of every row to its column and drops a row of empty cells, so it cannot produce the contract's plain output (two spaces between columns, nothing after the last cell, a hint row aligned under the detail). The type is about 35 lines and measures with lipgloss. |
