...
| Help and error pages | [fang](https://github.com/charmbracelet/fang); under evaluation | It would retire the help renderer in `internal/cli/help.go`, but only if it writes to the injected streams, stays plain on a pipe and leaves exit codes to the status table. |
