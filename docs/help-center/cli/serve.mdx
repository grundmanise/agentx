...
Only managed skills from a source you have added are checked, so a machine without any has nothing to check and serve fetches nothing. Every check lists every available update again, so the lines repeat on every check while the update is available. A skill a check could not check is an `update check:` warning instead, and the snapshot still shows its `candidate`. A check records what it found, so the snapshot that follows shows each skill's `candidate` and any `upstream removed` state. Nothing is ever applied.
