...
A source you have not fetched has no skills to search, and serve says so once as a warning. Run `agentx source add <url>` to fetch it. A source serve could not read is warned about the same way and tried again on the next scan, so it comes back without your having to restart serve.
