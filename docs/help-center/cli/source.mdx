...
A fetch updates what agentx knows about a source: the commit it holds and the skills inside it. Run it when you want to see what changed upstream. [`agentx skill check`](/cli/skill#check-for-updates) fetches the sources your managed skills came from on its own; nothing else refreshes a source you have not installed anything from yet.
