...
13 checks: 1 warning, 10 ok, 2 info
```

In a terminal the rows are coloured by status: green for `ok`, yellow for `warn`, red for `fail`, blue for `info`. Scan the first column for anything that is not a green check mark.
...
Each row is `ok`, `warn`, `fail` or `info`, marked `✓`, `!`, `✗` or `•`. A row with a fix shows it as a `hint:` line under the detail. The last line counts the rows by status.
