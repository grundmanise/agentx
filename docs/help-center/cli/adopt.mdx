...
Adopting adds any source you do not have yet, exactly as [`agentx source add`](/cli/source) would, and pins it to the branch or tag the lock file recorded. Then it records the version each skill was installed at and points that skill's import branch at it.
...
If the directory is really yours now rather than a copy of someone else's skill, fork it. A fork keeps your content with a history of its own, and agentx never pretends it came from upstream.
...
| The entry names a path that is not inside a repository | fix the entry, or install the skill instead |
